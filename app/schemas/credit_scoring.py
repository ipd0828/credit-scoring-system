...
class CreditScoringRequest(BaseModel):
    ...
    fico_range_low: int = Field(..., description="Нижний диапазон FICO скора", ge=300, le=850)
    fico_range_high: int = Field(..., description="Верхний диапазон FICO скора", ge=300, le=850)
    dti: float = Field(..., description="Соотношение долга к доходу", ge=0, le=100)
    
    # Дополнительные признаки
    revol_util: Optional[float] = Field(None, description="Использование возобновляемого кредита", ge=0, le=100)
    inq_last_6mths: Optional[int] = Field(None, description="Запросы за последние 6 месяцев", ge=0)
    delinq_2yrs: Optional[int] = Field(None, description="Просрочки за последние 2 года", ge=0)
    ...
    def validate_emp_length(cls, v):
        ...
        if v is not None:
            valid_values = ['< 1 year', '1 year', '2 years', '3 years', '4 years', 
...
class ModelPrediction(BaseModel):
    ...
    recommended_amount: Optional[float] = Field(None, description="Рекомендуемая сумма займа")
    ...
    features_importance: Optional[Dict[str, float]] = Field(None, description="Оценки важности признаков")
...
class CreditScoringFeedback(BaseModel):
    ...
    actual_outcome: int = Field(..., description="Фактический результат (0=одобрено, 1=отклонено)")
    feedback_notes: Optional[str] = Field(None, description="Дополнительные заметки обратной связи")
    user_id: Optional[str] = Field(None, description="Идентификатор пользователя, предоставившего обратную связь")
