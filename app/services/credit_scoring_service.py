...
class CreditScoringService:
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.error("Не удалось загрузить модель", error=str(e), model_path=self.settings.model_path)
            raise RuntimeError(f"Не удалось загрузить модель: {str(e)}")
    
    async def predict_credit_score(self, request: CreditScoringRequest) -> ModelPrediction:
        ...
        try:
            ...
            if prediction == 0:  # Одобрено
                recommended_amount = self._calculate_recommended_amount(request, probability)
    ...
    def _prepare_features(self, request: CreditScoringRequest) -> pd.DataFrame:
        ...
        expected_columns = [
            ...
            'dti', 'revol_util', 'inq_last_6mths', 'delinq_2yrs', 'pub_rec'
        ]
    ...
    def _engineer_features(self, features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'fico_range_low' in features and 'fico_range_high' in features:
            features['fico_avg'] = (features['fico_range_low'] + features['fico_range_high']) / 2
        
        # Calculate loan to income ratio
        if 'loan_amnt' in features and 'annual_inc' in features and features['annual_inc'] > 0:
            features['loan_to_income_ratio'] = features['loan_amnt'] / features['annual_inc']
        else:
            features['loan_to_income_ratio'] = 0
        
        # Convert employment length to numeric
        if 'emp_length' in features:
            features['emp_length_numeric'] = self._convert_emp_length_to_numeric(features['emp_length'])
    ...
    def _calculate_recommended_amount(self, request: CreditScoringRequest, probability: float) -> Optional[float]:
    ...
    def _get_feature_importance(self, features_df: pd.DataFrame) -> Optional[Dict[str, float]]:
        """Get feature importance if available."""
        try:
            if hasattr(self.model, 'feature_importances_'):
                importance_dict = dict(zip(features_df.columns, self.model.feature_importances_))
    ...
    async def log_prediction(
        ...
        try:
            ...
            logger.error("Failed to log prediction", error=str(e), request_id=request_id)
    ...
    async def process_feedback(self, feedback: CreditScoringFeedback):
        ...
        try:
            ...
            logger.error("Failed to process feedback", error=str(e), request_id=feedback.request_id)
    ...
    async def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "features_count": len(self.model.feature_importances_) if hasattr(self.model, 'feature_importances_') else None,
