...
class Settings(BaseSettings):
    ...
    model_path: str = Field(
        default="models/trained/credit_scoring_model.pkl", env="MODEL_PATH"
    )
    ...
    # Внешние API
    external_credit_api_url: Optional[str] = Field(
        default=None, env="EXTERNAL_CREDIT_API_URL"
    )
    external_credit_api_key: Optional[str] = Field(
        default=None, env="EXTERNAL_CREDIT_API_KEY"
    )
    ...
    enable_automatic_retraining: bool = Field(
        default=False, env="ENABLE_AUTOMATIC_RETRAINING"
    )
