...
def main():
    ...
    with tab1:
        ...
        **Используемые признаки (6):**
        ...
        # Prediction button
        if st.button("🔮 Получить Кредитный Скоринг", type="primary", use_container_width=True):
            # Валидация данных
            errors = []
            ...
            if status_code == 200 and result.get("success"):
                ...
                time_display = f"{processing_time:.0f}мс" if processing_time < 1000 else f"{processing_time / 1000:.2f}с"
                ...
                if prediction.get("features_importance"):
                    ...
                    importance_df["Признак"] = importance_df["Признак"].map(feature_translation)
                ...
                if "Проверьте консоль бэкенда" not in error_msg:
                    st.info("Проверьте консоль бэкенда для подробной информации об ошибке")
    ...
    with tab2:
        ...
        if model_info.get('model_loaded'):
            ...
            with col4:
                model_status = "Загружена" if model_info.get('model_loaded') else "Не загружена"
                st.metric("📈 Статус", model_status)

            # Дополнительные метрики если есть
            if any(key in model_info for key in ['accuracy', 'roc_auc', 'precision', 'recall']):
        ...
        features_info = {
            "Признак": ["Кредитный лимит", "Пол", "Семейное положение", "Возраст", "Статус платежей", "Образование"],
            ...
            "Тип": ["Числовой", "Категориальный", "Категориальный", "Числовой", "Категориальный", "Категориальный"]
        }
    ...
    with tab3:
        ...
        if model_info.get('model_loaded'):
            ...
            with col2:
                ...
                status_color = "✅ Загружена" if model_info.get('model_loaded') else "❌ Не загружена"
            ...
            if any(key in model_info for key in ['description', 'threshold', 'model_class']):
