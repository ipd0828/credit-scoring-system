...
def load_and_sample_data(
    data_path: str, sample_frac: float = 0.2, random_state: int = 42
...
def analyze_representativeness(
    ...
    bad_cat = len(
        report_categorical_summary[report_categorical_summary["Оценка"] == "Тревожно"]
    )
...
def analyze_target_variable(df: pd.DataFrame, target_col: str) -> Dict[str, Any]:
    ...
    def map_loan_status(status):
        ...
        # Хорошие займы -> 0
        if (
            "fully paid" in status_str
            or "current" in status_str
            or "good loan" in status_str
            or "no default" in status_str
