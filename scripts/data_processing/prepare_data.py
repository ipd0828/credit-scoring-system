...
class DataProcessor:
    ...
    def create_target_variable(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def map_loan_status(status):
            ...
            # Good loans
            if any(good in status_str for good in ['fully paid', 'current', 'good loan']):
                return 0
            # Bad loans
            else:
                return 1
    ...
    def engineer_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'emp_length' in df.columns:
            df['emp_length_numeric'] = df['emp_length'].apply(self._convert_emp_length_to_numeric)
    ...
    def select_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        feature_columns = [
            ...
            'fico_avg', 'loan_to_income_ratio', 'emp_length_numeric'
        ]
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Prepare data for credit scoring model")
