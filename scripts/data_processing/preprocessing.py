...
def remove_high_missing_columns(df: pd.DataFrame, threshold: float = 0.6) -> Tuple[pd.DataFrame, List[str]]:
    """
...
def remove_unnecessary_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    columns_to_drop = [
        'Unnamed: 0', 'id', 'url', 'policy_code', 'loan_status',
        'member_id', 'issue_d'
    ]
...
def handle_missing_values(df: pd.DataFrame, numeric_columns: List[str], 
    ...
    for col in numeric_columns:
        if col in df_clean.columns and df_clean[col].isnull().any():
            ...
            print(f"  {col}: заполнено {df_clean[col].isnull().sum()} пропусков медианой ({median_val:.2f})")
...
def create_preprocessor(numeric_columns: List[str], categorical_columns: List[str]) -> ColumnTransformer:
    """
    ...
    print(f"Препроцессор создан для {len(numeric_columns)} числовых и {len(categorical_columns)} категориальных признаков")
...
def split_data(df: pd.DataFrame, target_col: str = 'target', 
    ...
    print(f"  Обучающая выборка: {train_dist[0]:.1%} хороших, {train_dist[1]:.1%} плохих")
...
def create_sample_data(df: pd.DataFrame, sample_frac: float = 0.1, 
