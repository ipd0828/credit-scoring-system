...
def perform_grid_search(
    ...
    for i, param_grid in enumerate(
        tqdm(param_grids, desc="Поиск параметров", unit="модель")
    ):
        ...
        try:
            ...
            grid_search = GridSearchCV(
                pipeline, param_grid, cv=cv, scoring="roc_auc", n_jobs=n_jobs, verbose=1
            )
    ...
    for model_name, model_results in results.items():
        ...
        try:
            ...
            cv_scores = cross_val_score(
                best_model, X_test, y_test, cv=5, scoring="roc_auc"
            )
...
def save_tuned_models(
    ...
    for model_name, model_results in results.items():
        if "error" not in model_results and "best_estimator" in model_results:
            model_path = (
                output_path / f"tuned_{model_name.lower().replace(' ', '_')}.pkl"
            )
    ...
    if evaluation_results:
        ...
        if best_model_name in results and "best_estimator" in results[best_model_name]:
            ...
            print(
                f"  Лучшая модель ({best_model_name}) -> {output_path / 'best_tuned_model.pkl'}"
            )
...
def main():
    ...
    grid_search_results = perform_grid_search(
        X_train, y_train, preprocessor, param_grids
    )
