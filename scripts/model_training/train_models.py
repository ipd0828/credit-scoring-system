...
def load_processed_data(data_dir: str = "data/processed") -> Tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series, Any]:
    """
...
def train_and_evaluate_models(models: Dict[str, Pipeline], X_train: pd.DataFrame, 
                             X_test: pd.DataFrame, y_train: pd.Series, 
                             y_test: pd.Series, use_mlflow: bool = True) -> Dict[str, Dict[str, Any]]:
    """
...
def create_comparison_plot(results: Dict[str, Dict[str, Any]], output_dir: str = "models/artifacts") -> None:
    """
...
def create_roc_curves(probabilities: Dict[str, np.ndarray], y_test: pd.Series, 
...
def save_models(models: Dict[str, Pipeline], results: Dict[str, Dict[str, Any]], 
    ...
    if not results_df.empty:
        ...
        print(f"  Лучшая модель ({best_model_name}) -> {output_path / 'best_model.pkl'}")
