...
def load_models_and_data(models_dir: str = "models/trained", 
                        data_dir: str = "data/processed") -> Tuple[Dict[str, Any], pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
    """
...
def validate_single_model(model_name: str, model: Any, X_test: pd.DataFrame, 
...
def create_validation_plots(validation_results: List[Dict[str, Any]], 
...
def create_metrics_comparison_plot(validation_results: List[Dict[str, Any]], 
...
def create_roc_curves_plot(validation_results: List[Dict[str, Any]], 
                          output_path: Path) -> None:
...
def create_pr_curves_plot(validation_results: List[Dict[str, Any]], 
                         output_path: Path) -> None:
...
def create_confusion_matrices_plot(validation_results: List[Dict[str, Any]], 
                                  output_path: Path) -> None:
...
def generate_validation_report(validation_results: List[Dict[str, Any]], 
    ...
    with open(text_report_path, 'w', encoding='utf-8') as f:
        ...
        for result in valid_results:
            ...
            f.write(f"  False Positive Rate: {result['metrics']['false_positive_rate']:.4f}\n")
            f.write(f"  False Negative Rate: {result['metrics']['false_negative_rate']:.4f}\n")
