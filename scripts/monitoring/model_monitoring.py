...
class ModelMonitor:
    ...
    def detect_data_drift(self, 
        ...
        try:
            ...
            numeric_features = current_data[feature_columns].select_dtypes(include=[np.number]).columns
            ...
            if drift_scores:
                ...
                drift_results["overall_drift_detected"] = drift_results["drift_score"] > self.config["drift_threshold"]
            
            logger.info(f"Дрифт данных: {drift_results['overall_drift_detected']}, "
    ...
    def monitor_model_performance(self, 
        ...
        try:
            ...
            if metrics["f1_score"] < 0.7:
                ...
                logger.warning(f"Деградация производительности обнаружена: F1={metrics['f1_score']:.4f}")
    ...
    def detect_prediction_bias(self, 
                              X_test: pd.DataFrame,
                              y_test: pd.Series,
                    ...
                    if len(groups) > 1:
                        ...
                        if len(group_metrics) > 1:
                            ...
                            bias_results["attribute_bias"][attr] = {
                                ...
                                "bias_detected": max_diff > self.config["bias_threshold"]
                            }
                            
                            if max_diff > self.config["bias_threshold"]:
                                bias_results["bias_detected"] = True
                                logger.warning(f"Смещение обнаружено для атрибута {attr}: "
    ...
    def generate_monitoring_report(self, 
                                 ...
                                 sensitive_attributes: list = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if sensitive_attributes:
                bias_results = self.detect_prediction_bias(X_test, y_test, sensitive_attributes)
            ...
            # Общая оценка
            report["overall_status"] = "healthy" if not report["alerts"] else "issues_detected"
            
            logger.info(f"Отчет мониторинга сгенерирован: {len(report['alerts'])} алертов")
    ...
    def save_monitoring_report(self, report: Dict[str, Any], output_path: str) -> None:
        ...
        """
        try:
            output_file = Path(output_path) / f"monitoring_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def send_alerts(self, report: Dict[str, Any]) -> None:
        ...
        for alert in report["alerts"]:
            logger.warning(f"ALERT [{alert['severity'].upper()}]: {alert['message']} - {alert['details']}")
...
def main():
    ...
    required_files = [model_path, reference_data_path, current_data_path, test_labels_path]
    ...
    try:
        ...
        sensitive_attributes = ["grade", "emp_length"]  # Пример чувствительных атрибутов
