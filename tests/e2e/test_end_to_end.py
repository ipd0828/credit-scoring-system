...
"""
    ...
    def test_error_handling_and_recovery(self, full_project_setup):
        ...
        invalid_data = pd.DataFrame(
            {"invalid_col": [np.nan] * 100, "another_invalid": [None] * 100}
        )

        invalid_data.to_csv(
            project_dir / "data" / "raw" / "invalid_data.csv", index=False
        )
    ...
    def test_performance_benchmarks(self, full_project_setup, sample_processed_data):
        ...
        for size in data_sizes:
            ...
            test_data = sample_processed_data.sample(
                n=min(size, len(sample_processed_data)), random_state=42
            )
