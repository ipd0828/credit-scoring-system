...
'''
        ...
        with patch('sys.path', [str(project_dir)] + sys.path):
            ...
            with patch('sys.argv', ['eda.py']):
                try:
                    ...
                    assert (project_dir / "data" / "processed" / "eda_processed_data.csv").exists()
                    assert (project_dir / "data" / "processed" / "numeric_representativeness.csv").exists()
                    assert (project_dir / "data" / "processed" / "categorical_representativeness.csv").exists()
    ...
    def test_preprocessing_pipeline(self, temp_project_directory, sample_processed_data):
        ...
        with patch('sys.path', [str(project_dir)] + sys.path):
            ...
            with patch('sys.argv', ['preprocessing.py']):
                try:
                    ...
                    assert (project_dir / "data" / "processed" / "preprocessor.pkl").exists()
                    
                except Exception as e:
                    print(f"Preprocessing завершился с ошибкой (ожидаемо): {e}")
    
    def test_model_training_pipeline(self, temp_project_directory, sample_processed_data):
        ...
        joblib.dump(preprocessor, project_dir / "data" / "processed" / "preprocessor.pkl")
    ...
    def test_data_quality_monitoring(self, temp_project_directory, sample_processed_data):
        """Тест мониторинга качества данных."""
        project_dir = temp_project_directory
        
        # Создаем файл с данными
        sample_processed_data.to_csv(
            project_dir / "data" / "processed" / "X_train.csv", 
            index=False
        )
        
        with patch('sys.path', [str(project_dir)] + sys.path):
            from scripts.monitoring.data_quality_monitor import main as data_quality_main
            ...
            with patch('sys.argv', ['data_quality_monitor.py']):
                try:
                    ...
                    print(f"Data quality monitoring завершился с ошибкой (ожидаемо): {e}")
