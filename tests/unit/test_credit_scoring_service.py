...
class TestCreditScoringService:
    ...
    def mock_model(self):
        ...
        model = Mock()
        model.predict_proba.return_value = np.array([[0.3, 0.7]])  # 70% probability of rejection
    ...
    def test_calculate_recommended_amount(self, service_with_mock_model):
        ...
        recommended = service_with_mock_model._calculate_recommended_amount(request, 0.8)
        assert recommended == 5000.0
        
        # Medium risk - should reduce amount by 20%
        recommended = service_with_mock_model._calculate_recommended_amount(request, 0.6)
        assert recommended == 8000.0
        
        # Low risk - should give full amount
        recommended = service_with_mock_model._calculate_recommended_amount(request, 0.3)
