...
class TestPipeline:
    ...
    def test_main_success(self):
        ...
        with patch('scripts.run_pipeline.check_data_exists', return_value=True), \
             patch('scripts.run_pipeline.create_directories'), \
             patch('scripts.run_pipeline.run_script') as mock_run, \
    ...
    def test_main_no_data(self):
        ...
        with patch('scripts.run_pipeline.check_data_exists', return_value=False), \
             patch('sys.argv', ['run_pipeline.py']):
            result = main()
            assert result == 1
