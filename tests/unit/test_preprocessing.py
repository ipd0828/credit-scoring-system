...
class TestPreprocessing:
    ...
    def sample_data(self):
        ...
        data = {
            ...
            "emp_length": np.random.choice(
                ["< 1 year", "1 year", "2 years", "3 years"], 100
            ),
    ...
    def test_remove_high_missing_columns(self, sample_data):
        ...
        df_clean, dropped_columns = remove_high_missing_columns(
            sample_data, threshold=0.6
        )
    ...
    def test_handle_missing_values(self, sample_data):
        ...
        df_clean = handle_missing_values(
            sample_data, numeric_columns, categorical_columns
        )
    ...
    def test_split_data(self, sample_data):
        ...
        X_train, X_test, y_train, y_test = split_data(
            sample_data, target_col="target", test_size=0.2, random_state=42
        )
...
class TestPreprocessingEdgeCases:
    ...
    def test_all_missing_values(self):
        ...
        all_missing_df = pd.DataFrame(
            {"col1": [np.nan] * 100, "col2": [np.nan] * 100, "target": [0, 1] * 50}
        )

        # remove_high_missing_columns должен удалить все колонки кроме target
        df_clean, dropped_columns = remove_high_missing_columns(
            all_missing_df, threshold=0.5
        )
